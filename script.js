...
// ----------------------------------------------------
// 3. ДАННЫЕ ДЛЯ СТРАНИЦЫ "НАШ ЯЗЫК / ЛЕГЕНДА"
// ----------------------------------------------------
const LEGEND_DATA = {
            ...
            { phrase: "Пожалуйста → Пэжик" },
             { phrase: "Люблю тебя → Лавю, ловелас, ловстер, лувики" },
              { phrase: "Жестко → Джойстик" },
               ...
               { phrase: "Молодец → Молодчик баллончик бульончик" },
           { phrase: "Не знаю → Хэзе, Хэзибургер, Хэмбургер"},
            { phrase: "Вейп → Вайбер, попарить → Пэпэ" }
        ]
    },
    
    // 2. Имена и Звания (Прозвища)
    names: {
        title: "Наши прозвища ✨",
        items: [
            { phrase: "Миссис умчик и мистер умняо", info: "тупа мы" },
            { phrase: "Алёна матусевка принцесса котиковна", info: "мое полное имя" },
            { phrase: "Кубик нубик / Кубик рубЭГ", info: "пхахахпхпхпхха эгг" },
            { phrase: "Нубяо, нубчик, нубер", info: "используем чаще, чем собственные имена" },
            { phrase: "Червячок", info: "это я" },
         { phrase: "Червячище", info: "это ты" },
            { phrase: "Стрекозка", info: "когда я сделала реснички" },
            { phrase: "Картошечка", info: " когда я прислала кружочек" },
            { phrase: "Нублопончик, Нублоящер, Турбоящик, Нублопукер и тд", info: "варианты, как мы называем друг друга" }
        ]
    },

    // 3. Ритуалы и Словечки (Фразы-эмоции)
    rituals: {
        title: "Ритуалы и прикольные словечки 🤫",
        items: [
            { phrase: "Чики брики пальчик выкинь", info: "нужно обязательно показать сердечко" },
            { phrase: "Жёлтая машина", info: "обязательно чмок" },
            { phrase: "Макиношка, макиняо", info: "наша обычная суббота)" },
            { phrase: "Б бесит", info: "это когда все реально б бесит" },
            { phrase: "Отступ", info: "отстань от меня пэжэ" },
           { phrase: "Тыща вещичек", info: "все, что в моей комнате это тыща вещичек" },
            { phrase: "Гребани, долбани, нуб, черт, нос", info: "твоя фраза в любой непонятной ситуации" }
        ]
    },
    
    // 4. Воспоминания и Факты (Слова-ассоциации)
    memories: {
        title: "Воспоминания 🎮",
        items: [
    ...
    // 5. МЕМНЫЕ ПЕСНИ (ПЕРЕМЕЩЕНО В КОНЕЦ)
    memesongs: {
        title: "Наши песенки 🎶",
        items: [
            { phrase: "Лялипоп" },
            { phrase: "Бэбищак тутутутрутут" },
            { phrase: "Щайн брайн лайк э дайдмонд" },
            { phrase: "Абракадабра" },
            { phrase: "Нубяо сильный" },
            { phrase: "Ралли у" }
        ]
    }
};
...
// --- 6. Переключение Секций (ОБНОВЛЕННАЯ ФУНКЦИЯ) ---
...
function showSection(sectionId) {
    ...
    if (newActiveSection) {
        ...
        setTimeout(() => {
            ...
            // --- ДОБАВЛЕНИЕ ЛОГИКИ ДЛЯ НОВОЙ СТРАНИЦЫ ---
            if (sectionId === 'legend') {
                renderLegend(); 
            }
...
// --- 7. Интерактивные Flip Cards (БЕЗ ИЗМЕНЕНИЙ) ---
...
// --- 8. Эффект Параллакса (БЕЗ ИЗМЕНЕНИЙ) ---
...
// ----------------------------------------------------
// 9. ЛОГИКА МОДАЛЬНОГО ОКНА ФОТО / ГРУППОВОЕ ВИДЕО
// ----------------------------------------------------
...
// 9.1. ОБРАБОТЧИК ДЛЯ ГАЛЕРЕИ (Карточки) - ФОТО
...
// 9.2. ОБРАБОТЧИКИ ДЛЯ ХРОНИКИ (TIMELINE) - ФОТО
...
// 9.3. НОВЫЙ ОБРАБОТЧИК ДЛЯ ВИДЕО-ГРУПП (Включает навигацию)
...
// 9.4. ОБЩЕЕ ЗАКРЫТИЕ МОДАЛЬНОГО ОКНА ФОТО/ГРУППОВОГО ВИДЕО
...
// ----------------------------------------------------
// 10. ЛОГИКА ОДИНОЧНОГО ВИДЕО-МОДАЛЬНОГО ОКНА
// ----------------------------------------------------
